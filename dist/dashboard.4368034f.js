...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"dashboard/dashboard.js":[function(require,module,exports) {
...
var testDataDeserialized = JSON.parse(localStorage.getItem('testScores'));
var testDataKeys = Object.keys(testDataDeserialized);
var recentTests = testDataKeys.slice(-10); // Get the test scores for the most recent six tests.

var recentTestScores = [];

for (var i = Object.keys(testDataDeserialized).length - 1; i >= 0; i--) {
  var singleTestData = testDataDeserialized[testDataKeys[i]];
  var numberCorrect = singleTestData.filter(function (data) {
    return data !== 0;
  });
  var testScore = Math.round(100 * (numberCorrect.length / singleTestData.length));
  ...
  options: {
    ...
    scales: {
      y: {
        beginAtZero: true,
        max: 100[{
          afterDataLimits: function afterDataLimits(axis) {
            axis.max += 1;
          }
        }]
      }
...
displayMissedQuestions = function displayMissedQuestions() {
  ...
  var missedQuestions = JSON.parse(localStorage.getItem('missedQuestions')); // Create an object that contains all the missed questions and their
  // frequency. Example: {00001: 3, 00003: 5, 00002: 6}

  var missedQuestionsObject = {};

  var _iterator = _createForOfIteratorHelper(missedQuestions),
      _step;
  ...
  var missedQuestionsArray = Object.entries(missedQuestionsObject); // Sort the multidemensional array in descending order based on frequency.
  ...
  for (var _i = 0, _missedQuestionsArray = missedQuestionsArray; _i < _missedQuestionsArray.length; _i++) {
